...
export const MetricsGraph: React.FC<MetricsGraphProps> = ({ history, config }) => {
  ...
  useEffect(() => {
    ...
    // Draw MAR threshold line (Dashed Orange)
    ...
    // Draw EAR Line (Red if below threshold, White/Zinc otherwise)
    ctx.beginPath();
    ...
    ctx.fillStyle = history[lastIdx].ear < config.earThreshold ? '#ef4444' : '#ffffff';
    ctx.beginPath();
  ...
  return (
    <div
      ...
      className="bg-zinc-900/40 border border-zinc-800 p-4 font-mono"
    >
      <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
        ...
        <div className="flex items-center gap-4 text-[10px] uppercase tracking-wider">
          ...
          <div className="flex items-center gap-1.5 text-zinc-300">
            <span className="h-1.5 w-3 bg-blue-400" />
            <span>MAR (MOUTH)</span>
          </div>
        </div>
      </div>

      <div className="relative h-32 w-full overflow-hidden bg-zinc-950 border border-zinc-800">
        <canvas
          ref={canvasRef}
          width={700}
          height={128}
        ...
        {/* Left Y-axis labels */}
        <div className="absolute left-2 inset-y-2 flex flex-col justify-between text-[9px] font-mono text-zinc-600 pointer-events-none uppercase">
          <span>0.80</span>
          <span>0.50</span>
          <span>0.25</span>
          <span>0.00</span>
